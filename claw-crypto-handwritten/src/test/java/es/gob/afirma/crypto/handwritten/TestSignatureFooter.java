...
public class TestSignatureFooter {
	...
	public void testSimpleFooter() throws Exception {
		final byte[] jpg = AOUtil.getDataFromInputStream(JseUtil.class.getResourceAsStream("/IMG_5699243969525973523.jpg")); //$NON-NLS-1$
		try (
	...
	public void testSimpleHeader() throws Exception {
		final byte[] jpg = AOUtil.getDataFromInputStream(JseUtil.class.getResourceAsStream("/IMG_5699243969525973523.jpg")); //$NON-NLS-1$
		try (
			final OutputStream os = new FileOutputStream(File.createTempFile("CABECERA", ".jpg")); //$NON-NLS-1$ //$NON-NLS-2$
		) {
