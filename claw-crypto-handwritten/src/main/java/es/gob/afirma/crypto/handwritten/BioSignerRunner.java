...
public final class BioSignerRunner implements SignaturePadListener {
	 ...
	 * lista de firmas. */
	private final Map<String, Integer> signIdAssociation = new ConcurrentHashMap<>();
	Map<String, Integer> getAssociationMap () {
		return this.signIdAssociation;
	}

	/** Resultados de las firmas que se van haciendo. */
	private final Map<String, SignatureResult> sigResults = new ConcurrentHashMap<>();

	private final List<String> signIdList = new ArrayList<>();
	...
	Map<SignerInfoBean, SignatureResult> buildSrList() {
		...
		final Map<SignerInfoBean, SignatureResult> srList = new ConcurrentHashMap<>(this.sigResults.size());
