...
public final class WacomSignaturePad extends SignaturePad implements ITabletHandler,
	...
	private final List<PenData> penData = new ArrayList<>();
	...
	public WacomSignaturePad(final Frame frame, final String signId, final boolean activateBtns) throws SignaturePadException {
		...
		LOGGER.info("Resolucion actual de la pantalla: " + getToolkit().getScreenResolution()); //$NON-NLS-1$
        ...
        final String model = PadUtils.getTabletModel(this.tablet);
		...
		try {
			...
			if(!activateBtns) {
				final JButton b1 = new JButton(Messages.getString("WacomSignaturePad.1")); //$NON-NLS-1$
				b1.setBounds(
				...
				final JButton b2 = new JButton(Messages.getString("WacomSignaturePad.2")); //$NON-NLS-1$
				b2.setBounds(
				...
				final JButton b3 = new JButton(Messages.getString("WacomSignaturePad.3")); //$NON-NLS-1$
				b3.setBounds(
